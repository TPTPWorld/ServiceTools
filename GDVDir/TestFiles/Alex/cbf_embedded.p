...
thf(1,conjecture,( mvalid @ ((mimplies@((mbox@(mforall_vary__d_i@(^ [ X : $i ] : ((p@X))))))@(mforall_vary__d_i@(^ [ X : $i ] : ((mbox@(p@X))))))) )).
...
%thf(spec,logic,($modal:=[$constants:=$rigid,$quantification:=$cumulative,$consequence:=$global,$modalities:=$modal_system_K])).
%thf(f,type,p:$i>$o).
