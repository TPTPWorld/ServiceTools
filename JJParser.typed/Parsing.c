...
//----In TFF (for TFX) only numbers and distinct objects are done as terms.
//----Functions and constants are done as predicates. This allows formulae
//----as arguments.
(Language = tptp_tff && !CheckTokenType(Stream,number) &&
!CheckTokenType(Stream,distinct_object))) {
        THFTerm = NewTerm();
...
//----Trying to revert things that look like functions to type function. But
//----this leaves the principle symbol in the predicate list, and I fear that
//----shit will ensue.
//        if (THFTerm->TheSymbol.Formula->Type == atom) {
//    TERM TempTerm;
//            TempTerm = THFTerm->TheSymbol.Formula->FormulaUnion.Atom;
//            THFTerm->TheSymbol.Formula->FormulaUnion.Atom = NULL;
//            FreeTerm(&THFTerm,NULL);
//            THFTerm = TempTerm;
//        }
        return(THFTerm);
    } else {
