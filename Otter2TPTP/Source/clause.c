		  ...
		  struct clause *cl)
{
  ...
  int prev;
	...
	write_term_with_big_variables(fp, t,1000,&prev,"",0);
	free_rel(r);
	free_term(t);
      }
      else
	write_term_with_big_variables(fp, lit->atom,1000,&prev,"",0);
      lit = lit->next_lit;
      if (lit)
	fprintf(fp, " | ");
    }
...
void print_clause_in_tstp(FILE *fp,
		  struct clause *cl,char * Status,int Index)
{

/* Made all axioms because tptp4X does not like the list names */
/*  fprintf(fp, "cnf(%s_%d,%s,(\n    ",Status,Index,Status); */
  fprintf(fp, "cnf(%s_%d,%s,(\n    ",Status,Index,"axiom");
  print_clause(fp,cl);
  fprintf(fp, "   )).\n\n");
}
		   ...
		   struct list *lst,char * Status)
{
  ...
  if (!lst)
    ...
    cl = lst->first_cl;
    while (cl) {
      print_clause_in_tstp(fp, cl,Status,Index);
      cl = cl->next_cl;
      Index++;
    }
  }
