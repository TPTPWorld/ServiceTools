...
char * InfixToPrefix(char * Infix) {
    ...
    if (!strcmp(Infix,"*")) return("infix_times");
    if (!strcmp(Infix,"+")) return("infix_plus");
    if (!strcmp(Infix,"-")) return("infix_minus");
    if (!strcmp(Infix,"/")) return("infix_divide");
    if (!strcmp(Infix,"mod")) return("infix_mod");
    if (!strcmp(Infix,"#")) return("infix_hash");
    if (!strcmp(Infix,"<")) return("infix_less");
    if (!strcmp(Infix,">")) return("infix_greater");
    if (!strcmp(Infix,"<=")) return("infix_less_or_equal");
    if (!strcmp(Infix,">=")) return("infix_greater_or_equal");
    if (!strcmp(Infix,"==")) return("infix_identical");
    if (!strcmp(Infix,"=/=")) return("infix_non_identical");
    if (!strcmp(Infix,"@<")) return("infix_before");
    if (!strcmp(Infix,"@>")) return("infix_after");
    if (!strcmp(Infix,"@<=")) return("infix_before_or_equal");
    if (!strcmp(Infix,"@>=")) return("infix_after_or_equal");
    return("ERROR");
...
void write_term_with_big_variables(FILE *fp,
		struct term *t,
		int n,
		int *prev,char * Variables,int QuantifierVar)
{
  struct rel *r;
  struct term *t1;
  struct sym_ent *s;
  ...
  if (t->type == NAME) {
    ...
    if (str_ident(str, "$nil"))
      ...
      strcat(LookFor,str);
      strcat(LookFor,":");
      if (strstr(Variables,LookFor) != NULL) {
          str[0] = toupper(str[0]);
      }
      fprintf(fp, "%s", str);
	...
	write_term_with_big_variables(fp, r->argval, 0, prev,Variables,0);
	if (r->narg) {
	  fprintf(fp, " "); *prev = OTHER_PUNC;
	}
      }
	...
	fprintf(fp, ")"); *prev = OTHER_PUNC;
      ...
      fprintf(fp, "["); *prev = OTHER_PUNC;
      write_term_with_big_variables(fp, t->farg->argval, 1000, prev,Variables,0);
      t1 = t->farg->narg->argval;
      while (t1->sym_num == str_to_sn("$cons", 2)) {
	fprintf(fp, ","); *prev = OTHER_PUNC;
	write_term_with_big_variables(fp, t1->farg->argval, 1000, prev,Variables,0);
	...
	fprintf(fp, "|"); *prev = OTHER_PUNC;
	write_term_with_big_variables(fp, t1, 1000, prev,Variables,0);
	fprintf(fp, "]"); *prev = OTHER_PUNC;
      }
    }   /* end list */
    else {
	...
	write_term_with_big_variables(fp, t->farg->argval, na1, prev,Variables,0);
	...
	if (str_ident(s2, " "))
	  ...
	  *prev = (name_sym(str) ? NAME_SYM : SYM_SYM);
	write_term_with_big_variables(fp, t->farg->narg->argval, na2, prev,Variables,0);
	if (s->op_prec > n) {
	  fprintf(fp, ")"); *prev = OTHER_PUNC;
	}
    } else {
    fprintf(fp, "%s(", InfixToPrefix(str));
    *prev = OPEN_PAREN;
    write_term_with_big_variables(fp, t->farg->argval, na1, prev,Variables,0);
    fprintf(fp,",");
    *prev = OTHER_PUNC;
...
/*----Ensure principle symbols start lowercase */
    if (isupper(str[0])) {
      fprintf(fp,"s_");
    }
	fprintf(fp, "%s", str);
	...
	r = t->farg;
	while(r != NULL) {
	  write_term_with_big_variables(fp, r->argval, 1000, prev,Variables,0);
	  r = r->narg;
	  if(r != NULL) {
	    fprintf(fp, ","); *prev = OTHER_PUNC;
	  }
