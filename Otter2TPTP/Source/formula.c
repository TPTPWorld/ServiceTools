...
void print_formula_with_big_variables(FILE *fp,
		   struct formula *f,char * Variables)
{
  char op[MAX_NAME];
  struct formula *f1;
  int i;
  ...
  if (f == NULL)
    ...
    write_term_with_big_variables(fp, f->t, 1000, &i,Variables,0);
  }
  else if (f->type == NOT_FORM) {
    fprintf(fp, "~(");
    print_formula_with_big_variables(fp, f->first_child,Variables);
    fprintf(fp, ")");
  }
    ...
    if (f->quant_type == ALL_QUANT)
      ...
      fprintf(fp, "? [");
      write_term_with_big_variables(fp, f->t, 1000, &i,Variables,1);
    fprintf(fp, "] : ");
    print_formula_with_big_variables(fp, f->first_child,Variables);
    fprintf(fp, ")");
  }
    ...
    for (f1 = f->first_child; f1; f1 = f1->next) {
      print_formula_with_big_variables(fp, f1,Variables);
      if (f1->next)
	fprintf(fp, " %s", op);
    }
    fprintf(fp, ")");
  }
}  /* print_formula_with_big_variables */
...
void print_formula_list(FILE *fp,
			struct formula_ptr *p,char * Type)
{
    int Index = 1;
    char Variables[MAX_NAME];

  while (p != NULL) {
    strcpy(Variables,":");
/* Made all axioms because tptp4X does not like the list names */
/*    fprintf(fp,"fof(%s_%d,%s,\n    ",Type,Index++,Type); */
    fprintf(fp,"fof(%s_%d,%s,\n    ",Type,Index++,"axiom");
    print_formula_with_big_variables(fp, p->f,Variables); 
    fprintf(fp, ").\n\n");
    p = p->next;
  }
}  /* print_formula_list */
