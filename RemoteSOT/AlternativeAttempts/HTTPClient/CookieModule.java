...
public class CookieModule implements HTTPClientModule
{
	     ...
	     cookie_jar.isFile()  &&  cookie_jar.canWrite()))
	{
	    ...
	    Enumeration myEnum = Util.getList(cookie_cntxt_list,
	    ...
	    {
		Cookie cookie = (Cookie) myEnum.nextElement();
		if (!cookie.discard())
	...
	synchronized (cookie_list)
	{
	    ...
	    Enumeration myEnum = cookie_list.elements();
	    while (myEnum.hasMoreElements())
		cookies[idx++] = (Cookie) myEnum.nextElement();
