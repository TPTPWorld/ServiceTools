...
*/
tff_atomic_formula [TptpParserOutput out, boolean[] polarity]
  returns [TptpParserOutput.TffFormula af]
  { TptpParserOutput.AtomicFormula atom;
  }
  : atom = atomic_formula[out,polarity]
    {
      af = out.createTffAtomicFormula(atom);
    }
  ;
...
//----TFF formulae
tff_formula  [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  { TptpParserOutput.BinaryConnective bc;
    TptpParserOutput.TffFormula tff_2;
  }
  : tff = tff_unitary_formula[out]
    ( /* a unitary formula */
    | bc = binary_connective 
      tff_2 = tff_unitary_formula[out]
      { tff = out.createTffBinaryFormula(tff,bc,tff_2);
      }
    | ( AND
        tff_2 = tff_unitary_formula[out]
        { tff = out.createTffBinaryFormula(tff,
TptpParserOutput.BinaryConnective.And,tff_2);
        }
      )+
    | ( VLINE
        tff_2 = tff_unitary_formula[out]
        { tff = out.createTffBinaryFormula(tff,
TptpParserOutput.BinaryConnective.Or,tff_2);
        }
      )+
    | ( COLON
        tff_2 = tff_top_level_type[out]
        { tff = out.createTffBinaryFormula(tff,
TptpParserOutput.BinaryConnective.Type,tff_2);
        }
      )
    | ( ASSIGN
        tff_2 = tff_formula[out]
        { tff = out.createTffBinaryFormula(tff,
TptpParserOutput.BinaryConnective.Assign,tff_2);
        }
      )
    )
  ;

tff_unitary_formula  [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  { boolean[] polarity ={true};
    TptpParserOutput.TffFormula af;
  }
  : tff = tff_quantified_formula[out]
  | tff = tff_unary_formula[out]
  | LPR tff = tff_formula[out] RPR
  | tff = tff_atomic_formula[out, polarity] {
      if (!polarity[0]) tff = out.createTffNegationOf(tff);
    }
  ;

//----Unary connectives bind more tightly than binary
tff_unary_formula  [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  : TILDA tff = tff_unitary_formula[out] {
      tff = out.createTffNegationOf(tff);
    }
  ;

//---- Formula must be closed
tff_quantified_formula  [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  {TptpParserOutput.Quantifier q;
  List<TptpParserOutput.TffFormula> vars;
  }
  : q = quantifier 
    LSB
    vars = tff_variable_list[out] 
    RSB 
    COLON
    tff = tff_unitary_formula[out] { 
      tff = out.createTffQuantifiedFormula(q, vars, tff);
    }
  ;

tff_variable_list  [TptpParserOutput out]
  returns [LinkedList<TptpParserOutput.TffFormula> vars]
//----Returned list is reversed
  { String var;
    TptpParserOutput.TffFormula type;
    TptpParserOutput.TffFormula onevar;
  }
  : var = variable[out]
    ( COLON
      type = tff_top_level_type[out]
      { onevar = out.createTffVariableAtom(var);
        onevar = out.createTffBinaryFormula(onevar,
TptpParserOutput.BinaryConnective.Type,type);
      }
    | { onevar = out.createTffVariableAtom(var);
      }
    )
    ( { vars = new LinkedList<TptpParserOutput.TffFormula>();
      }
    | COMMA
      vars = tff_variable_list[out]
    )
...
tff_top_level_type [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  { TptpParserOutput.TffFormula rest;
  }
  : tff = tff_unitary_type[out]
    ( //----ATomic type
    | MAP
      rest = tff_atomic_type[out]
      { tff = out.createTffBinaryFormula(tff,
TptpParserOutput.BinaryConnective.Map,rest);
      }
    )
...
tff_unitary_type [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  : tff = tff_atomic_type[out]
  | ( LPR
      tff = tff_xprod_type[out]
      RPR
    )
  ;

tff_atomic_type [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  { String typeword;
  }
  : typeword = atomic_word[out]
  { tff = out.createTffAtom(typeword);
  }
  | typeword = atomic_defined_word[out]
  { tff = out.createTffAtom(typeword);
  }
  ;

tff_mapping_type [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  { TptpParserOutput.TffFormula rest;
  }
  : tff = tff_xprod_type[out]
...
tff_xprod_type [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  { TptpParserOutput.TffFormula tff_2;
  }
  : tff = tff_atomic_type[out]
    ( STAR
      tff_2 = tff_atomic_type[out]
      { tff = out.createTffBinaryFormula(tff,
TptpParserOutput.BinaryConnective.XProd,tff_2);
      }
    )+
  ;
...
//        tff_2 = tff_unitary_formula[out]
//        { tff = out.createTffBinaryFormula(tff,
//TptpParserOutput.BinaryConnective.Subtype,tff_2);
//        }
//      )+

tff_defn_constant [TptpParserOutput out]
  returns [TptpParserOutput.TffFormula tff]
  : tff = tff_formula[out]
  ;

//tff_sequent [TptpParserOutput out]
//  returns [TptpParserOutput.TffFormula tff]
//  { TptpParserOutput.TffFormula lhs;
//    TptpParserOutput.TffFormula rhs;
//  }
//  : lhs = tff_tuple[out]
//    SEQUENT
//    rhs = tff_tuple[out]
//    { tff = out.createTffBinaryFormula(lhs,
//TptpParserOutput.BinaryConnective.Sequent,rhs);
//    }
//  ;
...
formula_data [TptpParserOutput out]
  ...
  {
  TptpParserOutput.ThfFormula thf;
  TptpParserOutput.TffFormula tff;
  ...
  : dw:DOLLAR_WORD
     ...
     | {dw.getText().equals("$tff")}?  /* tff formula */
       LPR tff = tff_formula[out] RPR
                                 {t = out.createGeneralTffFormula(tff);}
...
theory_name  [TptpParserOutput out]
  ...
  | {lw.getText().equals("answers")}?   {str = new String("answers");}
  | /* error */
...
tptp_input [TptpParserOutput out]
  ...
  | {str.getText().equals("tff")}?  /* tff annotated formula */
     LPR
     nm = name[out] COMMA
     role = formula_role[out] COMMA
     tff = tff_formula[out]
     ann = annotations[out]
     RPR DOT
      {in=out.createTffAnnotated(nm, role, tff, ann, str.getLine());}
