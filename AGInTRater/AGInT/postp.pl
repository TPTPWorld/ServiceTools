...
sub canSubsume {
  ...
  #if one of the clauses if FOF, skip this step (can't subsume, need to normilize first)
  if (grep(/fof\(/, $Fj) || grep(/fof\(/, $Fi)) {
    return 0;
  }
  $command = "CompareClauses << EOF\n$Fj$Fi"."EOF\n";
#DEBUG print("============\n$command===========\n");
  open2(\*CHLD_OUT, \*CHLD_IN,"CompareClauses") ||
    die("ERROR: Cannot run ComapreClauses program: $!\n");
  print(CHLD_IN "$Fj$Fi");
  close(CHLD_IN);
  if (defined($OutputLine = <CHLD_OUT>)) {
    chomp($OutputLine);
    close(CHLD_OUT);
