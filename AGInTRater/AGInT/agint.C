...
//----Fuck Yury Puzis. Hash size must be at least 1.
  hash_size = symbolsNumber + symbolsNumber + 1;
...
/*  my old code (geoff's fix above)
	double co = CoOccurence[i * hash_size + j] / 2.0;
	double perAxi = CoOccurence[i * hash_size + i];
	double perAxj = CoOccurence[j * hash_size + j];	
	CoOccurence[i * hash_size + j] = 1 - co /(perAxi + perAxj - co);
*/
...
FLEMMA* FLEMMA_COLLECTION::hashLemma(char* NameString) {
  FLEMMA* nlemma;
  StringHash shash(NameString);
  while (Lemmas.find(shash) != Lemmas.end()) {
    shash.hash++;
  }
  Lemmas[shash] = nlemma = new FLEMMA(nextid++);
  return nlemma;
}

//makrs the parents of f as being good or bad parents
void FLEMMA_COLLECTION::markGoodBad(ANNOTATEDFORMULA f, bool good) {
  SuperString ParentNames;
  GetParentNames(f, ParentNames);
  set<FLEMMA* > parents;
  for (char* temp = strtok(ParentNames,"\n"); temp; temp = strtok(NULL,"\n")) {
    //skip non-formula constructs like "theorem(equality)"
    if (strchr(temp,'(') != NULL)
      continue;
    //insert into set to avoid registering duplicates
    parents.insert(get(temp));
  }
  //increament number of good/bad children for this parent
  for (set<FLEMMA *>::iterator i = parents.begin(); i != parents.end(); i++)
    good ? (*i)->goodChildren++ : (*i)->badChildren++;
}
...
FLEMMA* FLEMMA_COLLECTION::add(ANNOTATEDFORMULA f, LData &ld) {
  ...
  for (int flat_i = 0; flat_i < ld.complexity; flat_i++) {
    ...
    for (int flat_j = flat_i+1; flat_j < ld.complexity; flat_j++) {
      double surp = 0.0;
      if (fsig_hash[flat_i].first == -2) {
        //cerr << "% One of the symbols in one of the formulae is nowhere to be "
	//     << "found in the axioms. Skipping." << endl;
      } else {
        //augment surp. value (the actual formula is already in the array)
        surp = CoOccurence[fsig_hash[flat_i].first * hash_size + 
	  			  fsig_hash[flat_j].first];           
      }
      if (surp > 0)
        ld.surprisingness += surp;
    }
  }
  ...
  for (char* temp = strtok(ParentNames,"\n"); temp; temp = strtok(NULL,"\n")) {
    ...
    if (si == Lemmas.end()) {
      cerr << endl << "Failed to find parent: " << temp << " for formula " << NameString << endl;
...
void FLEMMA_COLLECTION::RewriteSignature(SYMBOLNODE treeRoot, 
  ...
  if (treeRoot->NumberOfUses > 0) {
    ...
    //cout << "symbol: " << treeRoot->NameSymbol << endl;
  }
...
//    cerr << "% Error: couldn't find symbol " << tnode->NameSymbol << ", ignoring" << endl;
    return -2;	
  }
  return loc;
}
